...
int main(){
    ...
    while (std::cin >> num >> unit){
        if ((unit == "cm") || (unit == "m") || (unit == "in") || (unit == "ft")) {
            ...
            std::cout << "The largest so far = " << largest << "m\n" << "The smallest so far = " << smallest << "m\n";
        } else {
    ...
    std::cout << "smallest = " << smallest << "m\n"
                ...
                << "sum of all values = " << sum << "m\nThe values entered in meters are: \n";

    // part c++98 algorithm
    // std::sort(stored_values.begin(), stored_values.end());

    //part of c++20 algorithm. A modern, more expressive and safer alternative to the one above
    std::ranges::sort(stored_values);

    for(double number : stored_values) std::cout << number << ",\t";
    std::cout << '\n';
...
double convert_to_m(double number, std::string unit){
    if(unit == "cm"){
        number /= 100;
    } else if (unit == "in"){
        number *= 0.0254;
